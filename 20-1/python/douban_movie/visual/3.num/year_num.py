...
path = './data/movie_detail_year.csv' # use your path
print("正在对", path, '分数分布')
...
def getScore(file):
    ...
    yearDict = json.loads(df_year.loc[:,'year'].value_counts().to_json())
    ...
    print("============\n")
    print("美国年份变化: ",[num for( _, num) in sortedDict_ind])
    print("============\n")
    print("印度年份变化: ",[num for( _, num) in sortedDict_usa])
    print("============\n")
